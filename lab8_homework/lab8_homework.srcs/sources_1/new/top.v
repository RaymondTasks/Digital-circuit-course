...
module Clk_5MHz_to_1Hz(
	...
	always @(negedge clk_5MHz or negedge reset) begin
		if(reset==0) begin
			clk_1Hz<=0;
			cnt<=0;
		end 
...
module top(
	...
	wire thresh0;
	wire [3:0] A0,A1;
	c_counter_binary_0 counter0 (.CLK(clk_5MHz),.CE(clk_1Hz),.THRESH0(thresh0),.Q(A0),.SCLR(reset));
	c_counter_binary_0 counter1 (.CLK(clk_5MHz),.CE(thresh0),.Q(A1),.SCLR(reset));
